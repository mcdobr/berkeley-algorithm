...
void slave_reply_to_poll(double start_time, int world_rank);

void slave_wait_for_master_poll(int world_rank);

void slave_send_current_time(double start_time, int world_rank);
...
int main(int argc, char *argv[]) {
    ...
    if (world_rank == MASTER_RANK) {
        ...
        slave_reply_to_poll(start_time, world_rank);
...
void slave_reply_to_poll(double start_time, int world_rank) {
    ...
    slave_send_current_time(start_time, world_rank);
}
...
void slave_wait_for_adjustment(int world_rank) {
    ...
    printf("Process [%d] received a delta of %.4lf\n", world_rank, delta);
}
